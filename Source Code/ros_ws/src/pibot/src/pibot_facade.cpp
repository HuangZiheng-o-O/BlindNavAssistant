...
PibotManager::PibotManager()
  ...
  // wait for service
  laser_listener_.waitForTransform("map", "laser_link", ros::Time(0), ros::Duration(1.0));
...
void PibotManager::onReceiveLDS(const sensor_msgs::PointCloud::ConstPtr msg) {
  ...
  try {
    laser_listener_.lookupTransform("map", "laser_link", ros::Time(0), transform);
    ...
    for (int i = 0; i < msg->points.size(); i++) {
      ...
      local_point.header.frame_id = "laser_link";
